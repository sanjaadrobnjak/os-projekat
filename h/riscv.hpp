...
public:
    ...
    // read register scause-opis razloga za prelazak u sis. rezim
    static uint64 r_scause();

    // write register scause
    static void w_scause(uint64 scause);

    // read register sepc-sacuvana vrednost reg pc u kor rezimu
    static uint64 r_sepc();

    // write register sepc
    static void w_sepc(uint64 sepc);

    // read register stvec-adresa prek rutine
    ...
    //sip sadrzi inf koji zahtevi za prekid su trenutno aktivni
    enum BitMaskSip
    {
    ...
    // supervisor trap-poziva ga procesor kad se pojavi neki zahtev za prekid ili neki izuzetak
